...
function AddWorkoutsComponent() {
  ...
  const duration = parseInt(localStorage.getItem('duration') || '0'); // Get the duration from local storage
  const weeks = Math.floor(duration / 7); // Number of full weeks
  const remainingDays = duration % 7; // Days remaining after full weeks
  
  const [currentWeek, setCurrentWeek] = useState(1); // Added currentWeek state
  ...
  const isWorkoutSelectedForCurrentWeek = (workoutId: any) => { // Added helper function
  ...
  const getAvailableDays = () => {
    let weekDays = [1, 2, 3, 4, 5, 6, 7];  // Monday to Sunday
  ...
  const handleSaveButton = () => {
    ...
    addWorkoutToPlan(planId, workoutIdsList, daysList)
      .then(result => navigate('/')) // Navigate to rolepage (or you can change this to your main page route)
      .catch(error => console.error("Error adding workouts to plan:", error));
  };
  
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl mb-4">Week {currentWeek}</h1> {/* Show the current week */}
      <div>
        <button
          disabled={currentWeek === 1}
          onClick={() => setCurrentWeek(currentWeek - 1)}
          className="bg-blue-600 text-white rounded px-4 py-2 m-2"
        >
          Previous Week
        </button>
        {
          currentWeek === weeks + (remainingDays > 0 ? 1 : 0) ? (
            <button
              ...
              className="bg-green-600 text-white rounded px-4 py-2 m-2"
            >
              Accept
            </button>
          ) : (
  ...
  <button key={day} className="bg-blue-600 text-white rounded px-4 py-2 m-1" onClick={() => handleDaySelection(day)}>
    {daysOfWeek[day - 1]}  {/* Display the name of the day */}
  </button>
))}
          </div>
        </div>
      ...
      <ul>
        {Array.isArray(workouts) && workouts.length > 0 ? (
          workouts.map((workout: any) => (
            <li
              key={workout.id}
              className={`mb-6 p-4 rounded-lg shadow-sm cursor-pointer ${isWorkoutSelectedForCurrentWeek(workout.id) ? 'bg-green-500' : 'bg-white'}`} // Using the helper function here
              onClick={() => toggleWorkout(workout.id)}
            >
              <div className="flex items-center">
                <img src={workout.image} alt={workout.title} className="w-16 h-16 rounded-full object-cover mr-4" />
                <div>
      ...
      <button onClick={handleSaveButton} className="bg-blue-600 text-white rounded px-4 py-2 mt-4">Save</button>
    </div>
  );
}
